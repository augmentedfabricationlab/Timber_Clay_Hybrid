...
class Ceilingsystem:
    ...
    def __advanced_length_correction(self):
        ...
        position = 0
        while self.primary_inside_support_length > self.ceiling_grids[1][0][position]:
            position += 1
        # //1 just to get a nicer number
        self.primary_inside_support_length = (self.ceiling_grids[1][0][position] + self.secondary_board_width/2) // 1 + 1
        self.primary_inside_support_dimensions[2] = self.primary_inside_support_length

        # outside momentum supports
        outside_grid = self.ceiling_grids[1][0]
