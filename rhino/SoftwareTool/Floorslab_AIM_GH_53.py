...
def floorslab_creation(self):
            ...
            # inside layer, primary span
                ...
                if self.advanced_setup and self.primary_inside_support_length > 0 and self.skipping:
                    ...
                    for i in range(len(self.floorslab_grids[0][0])-1, 0, -2):
                        ...
                        print(self.primary_inside_support_layers)
    ...
    def board_geometry_setup():
        for my_element in self.elements():
            ...
            if not my_board.perp:
                my_board.length_vector = my_dir1
                my_board.width_vector = my_dir2
            else:
...
def gluepoints(system):
    ...
    def board_intersection(brd1, brd2):
        ...
        if not brd1.vert_sup and not brd2.vert_sup:
            ...
            if distance_point_point(brd1.centre_point, int_pt) < brd1.length / 2 and \
                ...
                intersection_point = Point(int_pt[0], int_pt[1], brd1.centre_point[2])
            ...
            if len(segments) == 2 or len(segments) == 4:
                ...
                if vec02.length < max(vec01.length, vec03.length):
                    a = corner_pts_temp[1]
                    corner_pts_temp[1] = corner_pts_temp[2]
                    corner_pts_temp[2] = a

                # now make sure that the third point is right
                vec12 = Vector.from_start_end(corner_pts_temp[0], corner_pts_temp[1])
                vec13 = Vector.from_start_end(corner_pts_temp[0], corner_pts_temp[2])
                ...
                brd1.gluepoint_corners = [corner_pts_temp[0], corner_pts_temp[1], corner_pts_temp[2], corner_pts_temp[3]]
