...
def floorslab_creation(self):
    ...
    def __board_data_setup(advanced):
        def element_data_creator(dims, lay, global_c, layer_c, z, len_dir, w_dir, grid_pos, prim_sup, sec_sup, loc="centre"):
            ...
            if sec_sup or prim_sup:
                myElement.width = self.vertical_support_interlock + self.vertical_support_width
                if layer_c == 0:
                    myElement.grid_position -= myElement.width
                    myElement.grid_position += (self.vertical_support_interlock - self.vertical_support_width)
                else:
                    myElement.grid_position += myElement.width
                    myElement.grid_position -= (self.vertical_support_interlock - self.vertical_support_width)
            ...
            if layer == 0 or (layer == self.layer_no - 1 and layer % 2 == 0):
                ...
                for i in range(len(self.floorslab_grids[0][0])):
                    ...
                    element_data_creator(primary_board_outside_dimensions, layer, global_counter, layer_counter, z_value,
                                         self.primary_direction, self.secondary_direction, board_position,
                                         False, self.sec_vert_sup)
                    ...
                    for i in range(len(self.floorslab_grids[0][1])):
                        ...
                        if (self.timberboards[layer][i + 1].grid_position - self.timberboards[layer][i].grid_position >
                            ...
                            self.timberboards[layer].append(self.Timberboard(layer, global_counter, layer_counter,
                                                                             ...
                                                                             self.secondary_direction, self.floorslab_grids[0][1][i]), False)
            ...
            # inside layer, primary span
                ...
                if not self.skipping:
                    for i in range(len(self.floorslab_grids[0][0])):
                        element_data_creator(self.primary_board_inside_dimensions, layer, global_counter, i, z_value,
                                             ...
                                             self.floorslab_grids[0][0][i], False, self.sec_vert_sup)
                        global_counter += 1
                ...
                # dedensification on the inside
                    ...
                    if len(self.floorslab_grids[0][0]) % 2 != 1:
                        element_data_creator(self.primary_board_inside_dimensions, layer, global_counter, layer_counter,
                                             z_value, self.primary_direction, self.secondary_direction,
                                             self.floorslab_grids[0][0][-1], False, self.sec_vert_sup)
                        ...
                        for i in range(1, len(self.floorslab_grids[0][0]), 2):
                            ...
                            if (self.timberboards[layer][i // 2 + 1].grid_position - self.timberboards[layer][i // 2].grid_position >
                                ...
                                self.timberboards[layer].append(self.Timberboard(layer, global_counter, layer_counter,
                                                                                 ...
                                                                                 z_value, self.primary_direction,
                                                                                 self.secondary_direction, self.floorslab_grids[0][0][i], False,
                                                                                 False, "low"))
                                layer_counter += 1
                                self.timberboards[layer].append(self.Timberboard(layer, global_counter, layer_counter,
                                                                                 self.primary_inside_support_dimensions,
            ...
            # secondary span
                ...
                for i in range(0, len(self.floorslab_grids[1][0])):
                    element_data_creator(self.secondary_board_dimensions, layer, global_counter, layer_counter,
                                         ...
                                         self.floorslab_grids[1][0][i], self.prim_vert_sup, False)
...
Slabassembly.sec_vert_sup = True
