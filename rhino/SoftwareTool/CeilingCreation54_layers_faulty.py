...
class Floorslabsystem:
        ...
        for layer_no, board_layer in enumerate(self.timberboards):
            ...
            while i < len(my_board_layer)-1:
                ...
                if current_gap > self.vert_sup_gap_min:
                    ...
                    self.timberboards[layer_no].append(self.Timberboard(self.layers[layer_no], global_counter, layer_counter, [width, height, length],
    ...
    class Timberboard:
        def __init__(self, board_layer, identification, board_no_in_layer, board_dimensions,
                     length_dir, width_dir, grid_pos, position="centre", sup = False):
    ...
    class SlabLayer:
        def __init__(self, id, z_drop_pt, height, length):
            self.index = id
            self.z_drop_point = z_drop_pt
            self.characteristic_height = height
    ...
    def __board_data_setup(self, advanced):
        ...
        for layer in range(self.layer_no):
            ...
            if layer == 0 or layer == self.layer_no - 1:
                ...
                z_value += boardheight
                self.layers.append(self.SlabLayer(layer, z_value, boardheight, primary_length))

                for i in range(len(self.floorslab_grids[0][0])):
                    self.timberboards[layer].append(self.Timberboard(self.layers[-1], global_counter, layer_counter, self.primary_board_outside_dimensions,
                ...
                if advanced and self.primary_outside_support_length > 0:
                    for i in range(len(self.floorslab_grids[0][1])):
                        ...
                        if (self.timberboards[layer][i + 1].grid_position - self.timberboards[layer][i].grid_position >
                            ...
                            self.timberboards[layer].append(self.Timberboard(self.layers[-1], global_counter, layer_counter,
            ...
            # inside layer, primary span
                ...
                if not self.skipping:
                    for i in range(len(self.floorslab_grids[0][0])):
                        self.timberboards[layer].append(self.Timberboard(self.layers[-1], global_counter, i, self.primary_board_inside_dimensions,
                                                                         self.primary_direction, self.secondary_direction,
                                                                         self.floorslab_grids[0][0][i]))
                        global_counter += 1
                ...
                # dedensification on the inside
                    ...
                    for i in range(0, len(self.floorslab_grids[0][0]), 2):
                        self.timberboards[layer].append(self.Timberboard(self.layers[-1], global_counter, layer_counter, self.primary_board_inside_dimensions,
                                                                         self.primary_direction, self.secondary_direction,
                                                                         self.floorslab_grids[0][0][i]))
                        global_counter += 1
                        layer_counter += 1
            ...
            # secondary span
                ...
                z_value += boardheight
                self.layers.append(self.SlabLayer(layer, z_value, boardheight, secondary_length))
                for i in range(0, len(self.floorslab_grids[1][0])):
                    self.timberboards[layer].append(self.Timberboard(self.layers[-1], global_counter, layer_counter, self.secondary_board_dimensions,
                                                                     self.secondary_direction, self.primary_direction,
    ...
    def board_geometry_setup(self):
        for my_layer in range(self.layer_no):
            ...
            for my_board in self.timberboards[my_layer]:
                ...
                # height vector
                my_vec3 = Vector(0, 0, my_board.layer.z_drop_point - my_board.height / 2)
...
print(myFloorslab.weight_calculator(5, 460, 1800))
