...
def floorslab_creation(self):
    ...
    def box_update(elmnt):
        elmnt.board_frame = Frame(self.centre_point, self.length_vector, self.width_vector)
                ...
                if board.layer < layer_number:
                    ...
                    for i, other_brd in enumerate(self.elements()):
                        ...
                        if other_board.layer < layer_number - 1:
                            ...
                            board.glue_paths.append(gluepath_creator(my_glue_surface, self.gluepathwidth))
                            self.network.edge[board.global_count][i] = self.network.node[other_board.global_count]

    def component_stack_export():
        stack = []
        for brd in self.elements():
            board = brd[1]
    ...
    def component_instances_export():
        global_boards = []
        for brd in self.elements():
            board = brd[1]
    ...
    if input_check():
        ...
        setup_done = True
        grasshopper_draw()
        #pprint.pprint(component_instances_export())
        #pprint.pprint(component_stack_export())
        return
